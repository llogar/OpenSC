...
#define ISOAPPLET_API_VERSION_MINOR 0x07
...
isoApplet_init(sc_card_t *card)
{
	...
	/* Cards <= 0.5 did not report ECC capabilities, assume they do */
	if (drvdata->isoapplet_version <= 0x0005) {
		drvdata->api_features = rbuf[2];
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_EXT_APDU)
			card->caps |=  SC_CARD_CAP_APDU_EXT;
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_SECURE_RANDOM)
	...
	} else if (drvdata->isoapplet_version < ((ISOAPPLET_API_VERSION_MAJOR << 8) | ISOAPPLET_API_VERSION_MINOR)) {
		drvdata->api_features = rbuf[2];
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_EXT_APDU)
			card->caps |=  SC_CARD_CAP_APDU_EXT;
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_SECURE_RANDOM)
			card->caps |=  SC_CARD_CAP_RNG;
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_ECDSA_SHA1) {
			/* Curves supported by the pkcs15-init driver are indicated per curve. This
			 * should be kept in sync with the explicit parameters in the pkcs15-init
			 * driver. */
			flags = 0;
		...
		drvdata->api_features = ((unsigned int)rbuf[2] << 8) | rbuf[3];
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_EXT_APDU)
			card->caps |=  SC_CARD_CAP_APDU_EXT;
		if(drvdata->api_features & ISOAPPLET_API_FEATURE_SECURE_RANDOM)
			card->caps |=  SC_CARD_CAP_RNG;
		if(drvdata->api_features & (ISOAPPLET_API_FEATURE_ECDSA_SHA1 | ISOAPPLET_API_FEATURE_ECDSA_PRECOMPUTED_HASH | ISOAPPLET_API_FEATURE_ECDH)) {
			/* Curves supported by the pkcs15-init driver are indicated per curve. This
			 * should be kept in sync with the explicit parameters in the pkcs15-init
			 * driver. */
			flags = 0;
			...
			if (drvdata->api_features & ISOAPPLET_API_FEATURE_ECDSA_PRECOMPUTED_HASH)
			{
				flags |= SC_ALGORITHM_ECDSA_RAW;
				flags |= SC_ALGORITHM_ECDSA_HASH_NONE;
			}
			if (drvdata->api_features & ISOAPPLET_API_FEATURE_ECDH)
			{
				flags |= SC_ALGORITHM_ECDSA_RAW;
		...
		sc_log(card->ctx, "IsoApplet: Invalid card information block length.");
	...
	if(drvdata->api_features & ISOAPPLET_API_FEATURE_RSA_PAD_NONE)
	...
	if(drvdata->api_features & ISOAPPLET_API_FEATURE_RSA_4096)
	{
                            ...
                            u8 *out, size_t outlen)
{
	        ...
	        drvdata->sec_env_alg_ref == ISOAPPLET_ALG_REF_ECDSA_PRECOMPUTED_HASH)
	{
		...
		free(p);
	} else if ((drvdata->sec_env_alg_ref == ISOAPPLET_ALG_REF_RSA_PAD_NONE) || (drvdata->sec_env_alg_ref == ISOAPPLET_ALG_REF_RSA_PAD_PKCS1)) {
